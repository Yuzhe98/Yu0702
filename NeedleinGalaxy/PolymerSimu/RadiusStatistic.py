...
def Rhist(
    ...
    print("data ={")
    for i in range(txtnum):
        print("{%g, %g}," % (np.log(plen[i]), avalue[i]))
    print("};")
    arr_rescaled = arr
    for i in range(txtnum):
    ...
    ax7.errorbar(np.log(plen), avalue, yerr=varalue,fmt="o",markersize=2)  # s=30,
    #ax7.scatter(np.log(plen),avalue, s=30, marker="o")
    #-0.111651 + 0.639912 x
    ax7xaxis = np.linspace(start=-0.03,stop=7.25,num=1000,endpoint=True)
    ax7yaxis = lnk + nu*ax7xaxis
    ax7.text(0.2, 3.5, "$%g + %g ln(N) $"%(lnk,nu), color='green')
